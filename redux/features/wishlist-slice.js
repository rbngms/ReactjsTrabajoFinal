...
export const wishlistSlice = createSlice({
  ...
  reducers: {
    add_to_wishlist:(state,{payload}) => {
      ...
      if(productIndex >= 0){
        toast.error(`${payload.title} ya esta en tu lista de deseos`, {
          position: 'top-left'
        })
      }
      else {
        state.wishlist.push(payload);
        toast.success(`${payload.title} agregado a tu lista de deseos`, {
          position: 'top-left'
        })
      }
    },
    remove_wishlist_product:(state,{payload}) => {
      ...
      toast.error(`retirado de tu lista de deseos`, {
        position: 'top-left'
      })
    }
  },
